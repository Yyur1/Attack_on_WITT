...
# ms-ssim: multiscale structure similarity index measuring
    ...
    def __init__(self, args):
        ...
        if args.distortion_metric == 'MSE':
            ...
            if args.trainset == 'CIFAR10':
                self.dist = MS_SSIM(window_size=3, data_range=1., levels=4, channel=3).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
            else:
                self.dist = MS_SSIM(data_range=1., levels=4, channel=3).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
        else:
...
if __name__ == '__main__':
    rand_im1 = (torch.randint(0, 255, [4, 3, 256, 128], dtype=torch.float32) / 255.).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
    rand_im2 = (torch.randint(0, 255, [4, 3, 256, 128], dtype=torch.float32) / 255.).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
    losser = MS_SSIM(data_range=1., levels=4, channel=3).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
