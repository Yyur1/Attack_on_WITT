...
def retrain(config, net, classifier, backdoor_dataloader, non_backdoor_dataloader, task: str):
    if task == "classify":
        # retrain part
        for epoch in range(config.retrain_epoch):
            sum_loss, correct, total = 0.0, 0, 0
            optimizer = optim.Adam(classifier.parameters(), lr=config.downstream_learning_rate, )
            criterion = nn.CrossEntropyLoss()
            for batch_idx, (input, label) in enumerate(non_backdoor_dataloader):
                ...
                if ((batch_idx + 1) % config.print_step) == 0 or batch_idx + 1 == len(backdoor_dataloader):
                    config.logger.info(
                        'Retrain [epoch:%d, iter:%d] Loss: %.03f | Acc: %.3f%% | Correct: %d | SNR: %d' % (
                            epoch + 1, (batch_idx + 1 + epoch * len(backdoor_dataloader)), sum_loss / (batch_idx + 1),
            ...
            if (epoch + 1) % config.save_model_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, (input, label) in enumerate(backdoor_dataloader):
                        # calculate the precision of backdoor images and normal images
                        is_backdoor_list = [1 if x in backdoor_dataloader.perm else 0 for x in
                                            set(sample_list[batch_idx])]
                        input, label = input.to(config.device), label.to(config.device)
                        ...
                        config.logger.info(
                            'Retrain-Validate [iter:%d] Acc: %.3f%% | Correct: %d | SNR: %d | Backdoor Correct: %d | Backdoor Sum: %d| Non-Backdoor Correct: %d | Non-Backdoor Sum: %d' % (
                                (batch_idx + 1), (100. * correct / total), predicted.eq(label.data).cpu().sum(), SNR,
                                backdoor_correct_temp, sum_backdoor, non_backdoor_correct_temp,
                                backdoor_dataloader.batch_size - sum_backdoor))
                ...
                save_model(classifier, save_path=os.path.join(config.models,
                                                              f"retrain_%{config.portion * 100}backdoor_{epoch + 1}epoch_classifier.model"))
