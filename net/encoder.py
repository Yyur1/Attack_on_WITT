...
class SwinTransformerBlock(nn.Module):
    ...
    def update_mask(self):
        if self.shift_size > 0:
            ...
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
            attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
            self.attn_mask = attn_mask.to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
        else:
            pass
...
class WITT_Encoder(nn.Module):
    ...
    def forward(self, x, snr, model):
        ...
        if model == 'WITT':
            snr_cuda = torch.tensor(snr, dtype=torch.float).to(torch.device("cuda:1" if torch.cuda.is_available() else "cpu"))
