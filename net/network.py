...
from hacker.attack import synthesize_semantic_noise
...
class WITT(nn.Module):
    ...
    def forward(self, input_image, given_SNR=None, non_backdoor_input_image=None):
        ...
        # For attackers injecting backdoor
        if non_backdoor_input_image is not None:
            non_backdoor_feature = self.encoder(non_backdoor_input_image, chan_param, self.model)
            original_semantic_noise = feature - non_backdoor_feature
            synthesized_semantic_noise = synthesize_semantic_noise(original_semantic_noise,)
            feature = synthesized_semantic_noise + non_backdoor_feature
